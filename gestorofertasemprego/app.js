...
function printView(id){document.querySelectorAll(".view").forEach(v=>v.classList.remove("printing"));$(id).classList.add("printing");window.print();setTimeout(()=>$(id).classList.remove("printing"),500)}$("btnSimularPesquisa").addEventListener("click",()=>{const fonte=$("fFonte").value,loc=$("fLocalidade").value||"Seia";$("resultadosPesquisa").innerHTML=`<h3>Resultados simulados</h3><p>Na versão final, este botão fará a pesquisa real nas plataformas configuradas e trará apenas ofertas dentro do âmbito geográfico.</p><div class="found"><div><h4>Oferta encontrada em ${esc(fonte)}</h4><p>Local: ${esc(loc)} · Estado: pendente de validação humana</p></div><button class="primary" id="btnImportFake">Importar para validação</button></div>`;$("btnImportFake").addEventListener("click",()=>{const o={id:crypto.randomUUID(),titulo:"Oferta importada para validação",entidade:"",localidade:loc,concelho:loc,fonte:fonte,referencia:"",link:"",dataOferta:"",dataConsulta:todayPT(),dataLimite:"",contrato:"",horario:"",remuneracao:"",estado:"pendente",dataInativacao:"",motivoInativacao:"",dataValidacao:"",dataExportPdf:"",dataExportFacebook:"",importadaPor:"Técnico não identificado",validadaPor:"",exportPdfPor:"",exportFacebookPor:"",inativadaPor:"",contacto:"",origemContacto:"",resumoFacebook:"",funcoes:"",requisitos:"",condicoes:"",observacoes:"Resultado simulado. Substituir por dados reais e verificáveis."};offers.unshift(o);currentId=o.id;saveOffers();showView("editor")})});refreshAll();renderFacebook();renderPdf();
